...
public class LogsParserService implements LogsParser<ParsedLogs> {
    ...
    @Override
    public ParsedLogs parseExperimentFile(File file, ObjectId experimentId, ImportDetails importDetails) throws IOException {
        var experimentConfiguration = getConfiguration(experimentId);
        var experimentEvaluation = getEvaluation(experimentConfiguration.getId());
        ...
        dataPoints.forEach(point ->
                point.addTag(InfluxDefaults.CommonTags.EVALUATION_ID, experimentEvaluation.getId().toHexString())
        );

        return new ParsedLogs(defaultLogsBucketName, dataPoints, experimentConfiguration, experimentEvaluation);
    }
    ...
    private ExperimentConfiguration getConfiguration(ObjectId experimentId) {
        // todo make its ID generated by db, without this its null and evaluation doesnt't have it
        var configurationId = new ObjectId();
    ...
    private ExperimentEvaluation getEvaluation(ObjectId configurationId) {
        return new ExperimentEvaluation(new ObjectId(), configurationId, Instant.now());
    }
}
