...
@Profile("mongo")
public class MongoLogsParser implements LogsParser<MongoLog> {
    ...
    public boolean saveLogsToDatabase(String rawLogs) {
        ...
        Map<MongoLogType, List<MongoLog>> logs = parseLogs(rawLogs);
    ...
    private Map<MongoLogType, List<MongoLog>> parseLogs(String rawLogs) {
        this.workplaceParametersSaved = false;
        String[] logs = rawLogs.split("\n");
        Map<MongoLogType, List<MongoLog>> mapReturned = new HashMap<>();
        mapReturned.put(MongoLogType.BEST_SOLUTION, new LinkedList<>());
        mapReturned.put(MongoLogType.SUMMARY, new LinkedList<>());
        mapReturned.put(MongoLogType.WORKPLACE, new LinkedList<>());
